...
// 			connections.pop();
			...
			if( rel.indexOf( searchString ) == -1 ){//如果两节点还不存在关系
				...
				ajaxReqConn("/busi/migJobContentBUSIlink.action", 
					"{jobId:'"+jobId+"',fromTask:'"+start.id+"',toTask:'"+motask.id+"'}" );
...
//         alert( "up x:"+ this.attr("x") + ", y: "+ this.attr("y") );
		if( this.ox != this.attr("x") || this.oy != this.attr("y") ){
			ajaxReq("/curd/migJobContentCURDmodifyCoords.action", 
        		"{jobId:'"+jobId+"',taskId:'" + this.id + "',coords:'"+this.attr("x") + ","+ this.attr("y")+"'}" );
		}
    };
    //创建绘图对象
...
// 	alert("obj.id 1:"+obj.id);
    obj.taskId = id;
//     alert("obj.id 2:"+obj.id);
    obj.drag(move, dragger, up);
    obj.dblclick(function () { app.f_modifyPara_open_window(jobId, this.taskId, this.zindex ) });
// 	obj.click( clickTask );
...
function createNode2(x, y, node ){
	...
	for(var i = 0,j = tpl.length; i < j; i++ ){
		para[new Number(tpl[i].id.paraId) ]= (tpl[i].defValue==undefined?"":tpl[i].defValue);
	}
...
// 	JSON.parse(text);		eval ("(" + text + ")");	text2json obj or array
   	$.ajax({
	   type: "POST",
	   url: getContextPath()+"/curd/migTaskInfoCURDaddRetId.action",
	   data: {
...
// 	   async: false,//default:true
	   success: function( oResponse ){// oResponse.id is task id
			for(var i = 0,j = tpl.length; i < j; i++ ){
				var param = {};
				param.id = {};
				param.id.comId = oResponse.comId;
		    ...
		    var task = {
					...
					coords : x+","+y, zb : { x: x, y : y },
		  			t : {
		  				...
		  				taskName : node.text+"-任务"+newCtrlNum, 
		  				taskNode : node.attributes.type, 
		  				comId:"", taskAuthor : "", taskRemark : "", taskUpdate : ""
		  			},
		  			paras: paras,
		  			para: para
			};
			...
			tasks.push( task );
			shapes.push( shape );
			txts.push( r.text( x + rect.x, y + rect.y, node.text+"-任务"+newCtrlNum ) );
...
<body rightmargin="0" bottommargin="0" leftmargin="0" topmargin="0" class="easyui-layout">
	...
	<div id="mm" class="easyui-menu" style="width:120px;">
		...
		<div data-options="disabled:false" onclick="javascript: unlink('DB','DE')">删除节点连接</div>
	</div>
	
	<div id="taskContextMenu" class="easyui-menu" style="width:120px;">
		<div data-options="disabled:false" onclick="javascript: alert(this.id);app.f_modifyPara_open_window(jobId, this.id, this.zindex )">编辑作业入口</div>
		<div data-options="disabled:false" onclick="javascript: unlink('DB','DE')">删除所有该作业入口的副本</div>
	</div>
	
	<!-- 控件参数修改界面 -->
	<div id="modifyPara"></div>
	<!--
	<div id="modifyPara_dlg-buttons" >
	    <a href="#" class="easyui-linkbutton" iconCls="icon-ok" onclick="javascript: app.modifyPara()">确定</a>
		<a href="#" class="easyui-linkbutton" iconCls="icon-cancel" onclick="javascript: $('#modifyPara').dialog('close')">取消</a>
	</div>  -->
...
function unlink(fromTask, toTask){
