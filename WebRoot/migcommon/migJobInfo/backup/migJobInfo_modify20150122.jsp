...
$(function () {
    ...
    move = function (dx, dy) {
		...
		var att2 = { x: att.x + rect.x, y: att.y + +rect.y };
		txts[this.zindex].attr(att2);
		
		//一个conn只有一个起点，一个终点
        for (var i = connections.length; i--; ) {
        	//只更新需要更新的关系
        	if( connections[i].start==this.id || connections[i].end==this.id  ){
//         		alert(connections[i].start+"--->"+connections[i].end);
...
//     var r = Raphael("holder", $('#workspace').width()-5, $('#workspace').height()-5 );
  ...
  */
	...
	for( var i=0, j=tasks.length; i<j; i++ ){
		...
		shape.zindex = i;
		shapes.push( shape );
		txts.push( r.text( tasks[i].zb.x+rect.x, tasks[i].zb.y+rect.y, tasks[i].t.taskName ) );
	}
...
//         		alert( tasks[m].id.taskId+"=="+postpos[i]+" : "+(tasks[m].id.taskId == postpos[i]) );
    ...
    for (var i = 0, j = os.length; i < j; i++) {
    	...
    	conn.start = os[i].obj1.id;
    	conn.end = os[i].obj2.id;
    	connections.push( conn );
    }
});
/** 格式化子任务图标  **/
fmt.fmtShape = function (obj, color, id, txt){
...
function createNode2(x, y){
    ...
    fmt.fmtShape( shape, color, newCtrlNum,"任务"+newCtrlNum);
