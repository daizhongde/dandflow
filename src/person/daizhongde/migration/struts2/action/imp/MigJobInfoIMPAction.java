...
import person.daizhongde.migration.spring.jdbc.MigJobInfoIMPService;
 ...
 * 
 */
public class MigJobInfoIMPAction extends BaseAction  {
	/**
	 * 
	 */
	...
	private MigJobInfoIMPService dataService;
	 ...
	 * 
	 */
	public String importJobXLS() throws Throwable{
...
//		System.out.println("upload.getAbsolutePath():"+upload.getAbsolutePath());
		...
		try
		{
			dataService.importJobXLS(upload, uploadFileName, uploadContentType, _, super.getLoginUser() );
			log.info("导入job文件完成!");
	...
	//		String tempFAbsDir = INIT.tempFileDirectory+"import/excel/";
			...
			String tempFAbsName = absConstant.getEXPfileName() + stm.format(new Date()) + "."+ExtensionName;
	 ...
	 * 
	 */
	public String importKPIXLS() throws Throwable{
		log.debug("begin upload mapping file-----------------------");
		log.debug("==========" + getUploadFileName());
		log.debug("==========" + getUploadContentType());
		log.debug("==========" + getUpload());
...
//			dataService.importKPIXLS(upload, uploadFileName, uploadContentType, _,  super.getLoginUser() );
			...
			dataService.importKPIXLS(upload, targetFAbsPath, uploadContentType, _,  super.getLoginUser() );
			
			log.info("导入KPI脚本文件完成!");
	...
	//		String tempFAbsDir = INIT.tempFileDirectory+"import/excel/";
//			SimpleDateFormat stm=new SimpleDateFormat("yyMMdd-HHmmss-S");
			String tempFAbsName = "KPI-" + stm.format(new Date()) + "."+ExtensionName;
	...
	//		System.out.println("tempFAbsDir:"+tempFAbsDir);
		...
		Map map = new HashMap(2);
		map.put("success", Boolean.TRUE );
		map.put("msg", "Import KPI Success！");
...
//		this.customColumns = customColumns;
	...
	public void setDataService(MigJobInfoIMPService dataService) {
