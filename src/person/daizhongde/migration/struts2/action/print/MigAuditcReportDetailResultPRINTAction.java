 ...
 * @date 2015-01-07
 */
public class MigAuditcReportDetailResultPRINTAction extends BaseAction {
...
//		int offset = (page - 1) * pageSize;// 第一条记录的索引,offset begin from 0, page begin from 1
		...
		String exportDiff = ((SQLNode)absConstant.getSQLDOC().getExport()
				.get("exportDiff")
			).getSQL();
	...
	public String printGbsubdomainReport() {
//		int pageSize = Integer.parseInt(rows==null?"0":rows.get(0).toString());// pageSize
//		int offset = (page - 1) * pageSize;// 第一条记录的索引,offset begin from 0, page begin from 1
		...
		String exportGroupbysudomain = ((SQLNode)absConstant.getSQLDOC().getExport()
				.get("exportGroupbysudomain")
			).getSQL();
		
		SQLAssembleQ sqlA = new SQLAssembleQ(
				absConstant.getSQLDOC(),
				exportGroupbysudomain,
				jsonObject.getJSONObject("condition"), 
				jsonObject.getJSONObject("operator"),
				absConstant.getColumnTypes(),
				absConstant.getFront2col(),
