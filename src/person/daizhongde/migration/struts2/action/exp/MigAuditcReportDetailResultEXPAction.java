...
import person.daizhongde.migration.spring.service.exp.MigAuditcReportDetailResultService;
 ...
 * 
 */
public class MigAuditcReportDetailResultEXPAction extends BaseAction  {
	/**
	 * 
	 */
...
//	private BaseExportService dataService;
	private MigAuditcReportDetailResultService dataService;
...
//		TimestampFormat	: "",////or "YYYYMMDD","YYYY-MM-DD-HHNNSS"
		...
		case "xls": iso88951 = cv.getGBK2ISO( fileName + (++countXLS) + Timestamp + ".xls");
			break;
		case "txt": iso88951= cv.getGBK2ISO( fileName +(++countTXT) + Timestamp + ".txt");
			break;
	 ...
	 * ConstMigAuditcReportDetailResult.Export.export  --ExportXLS()
	 * ConstMigAuditcReportDetailResult.Export.exportDayReport  --ExportDayReportXLS()
	 * ConstMigAuditcReportDetailResult.Export.exportMonthReport  --ExportMonthReportXLS()
	 * @throws Throwable 
	 * 
	 * 
	 * 
	 ...
	 * 同步导出(以提交form的形式导出)
	 * 
	 * @see person.daizhongde.migration.struts2.action.exp.MigAuditcReportDetailResultEXPAction#ExportXLS()
	 * @throws Throwable 
	 * 
	 */
	 ...
	 * 
	 */
	public String exportXLS_Diff() throws Throwable{
		...
		try
		{
			...
			AbstractConstant absConstant = new ConstMigAuditcReportDetailResult();
			String exportDiff_SQL = ((SQLNode)absConstant.getSQLDOC().getExport()
					.get("exportDiff")
				).getSQL();
			
//			List columnNames = JSONArray.fromObject("[\"dryrun_name\",\"DOMAIN2\",\"AUDIT_ITEM\",\"SRC_COUNT\",\"DST_COUNT\",\"Diff_Rate2\"]");
			
			//EXPORT.xlsExportOptions is globel option
			this.tempFAbsPath = dataService.exportXLS_Diff(jdata, _,
					customColumns,
				EXPORT.xlsExportOptions, exportDiff_SQL );
		}
		catch(Exception e)
		{
			e.printStackTrace();
	 ...
	 * 
	 */
	public String exportXLS_gbsubdomain() throws Throwable{
		...
		try
		{
			...
			AbstractConstant absConstant = new ConstMigAuditcReportDetailResult();
			String querygbsubdomain_SQL = ((SQLNode)absConstant.getSQLDOC().getExport()
					.get("exportGroupbysudomain")
				).getSQL();
			
//			List columnNames = JSONArray.fromObject("[\"dryrun_name\",\"DOMAIN2\",\"AUDIT_ITEM\",\"SRC_COUNT\",\"DST_COUNT\",\"Diff_Rate2\"]");
			
			//EXPORT.xlsExportOptions is globel option
			this.tempFAbsPath = dataService.exportXLS_gbsubdomain(jdata, _,
					customColumns,
				EXPORT.xlsExportOptions, querygbsubdomain_SQL );
		}
		catch(Exception e)
		{
			e.printStackTrace();
...
//		this.customColumns = customColumns;
	...
	public void setDataService(MigAuditcReportDetailResultService dataService) {
