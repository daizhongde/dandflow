 ...
 * @date 2015-01-07
 */
public class MigJobProcessCBBQUERYAction extends BaseAction {
	/**
	 * 
	 */
	...
	public String dfind() {
		...
		if( StringUtils.isNotEmpty(job_ins_id) ){
			condition.put("job_ins_id", job_ins_id );
			operator.put("job_ins_id", Operator.EQUAL );
		}
		
		if( StringUtils.isNotEmpty(q) ){
			condition.put("node_name", q );
			operator.put("node_name", Operator.CONTAIN );
		}
	...
	public String dfindArray() {
		...
		JSONObject jsonObject = JSONObject.fromObject( super.getJdata() );
//		SQLAssemble2 sqlA = new SQLAssemble2(jsonObject, "MigJobProcess");
		AbstractConstant absConstant = new ConstMigJobProcess();
...
//		this.sort = String.valueOf(Integer.valueOf(sort) + 1 );//where datatable's data is array use NO. to order, because front column index begin from 0 ,so plus one
	...
	public String getJob_ins_id() {
		return job_ins_id;
	}

	public void setJob_ins_id(String job_ins_id) {
		this.job_ins_id = job_ins_id;
	}
