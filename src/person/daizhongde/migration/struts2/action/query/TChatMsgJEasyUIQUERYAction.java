 ...
 * @date 2015-01-07
 */
public class TChatMsgJEasyUIQUERYAction extends BaseAction {
	...
	protected TChatMsgService dataService;
	...
	public String dfind_Offline() {
		...
		SQLAssembleQ sqlA = new SQLAssembleQ(
				absConstant.getSQLDOC(),
				absConstant.getSQLDOC().getQuery().get("queryOfflineMsg").getSQL(),
				jsonObject.getJSONObject("condition"), 
				jsonObject.getJSONObject("operator"),
				absConstant.getColumnTypes(),
				absConstant.getFront2col(),
	...
	public String dfindArray() {
		...
		JSONObject jsonObject = JSONObject.fromObject(jdata);
//		SQLAssemble2 sqlA = new SQLAssemble2(jsonObject, "TChatMsg");
		AbstractConstant absConstant = new ConstTChatMsg();
	...
	public String dfindTotal_Offline() {
		// 当jdata.condition为空 没有where条件
		// 在这里读配置文件sql并组装sql的where条件
		JSONObject jsonObject = JSONObject.fromObject(jdata);
		AbstractConstant absConstant = new ConstTChatMsg();
...
//			JSONObject jsonObject = JSONObject.fromObject(jdata);
//			JdataChecker checker = new JdataChecker(jsonObject, "TChatMsg");
...
//	}
	...
	public void setDataService(TChatMsgService dataService) {
