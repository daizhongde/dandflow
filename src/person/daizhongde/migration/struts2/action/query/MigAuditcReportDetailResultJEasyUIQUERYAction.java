 ...
 * @date 2015-01-07
 */
public class MigAuditcReportDetailResultJEasyUIQUERYAction extends BaseAction {
	...
	protected MigAuditcReportDetailResultService dataService;
	 ...
	 * @return
	 */
	public String dfindGroupbysudomain() {
		int pageSize = Integer.parseInt(rows==null?"0":rows.get(0).toString());// pageSize
		int offset = (page - 1) * pageSize;// 第一条记录的索引,offset begin from 0, page begin from 1
		...
		String queryDiff = ((SQLNode)absConstant.getSQLDOC().getQuery()
				.get("queryGroupbysudomain")
			).getSQL();
		
		SQLAssembleQ_GroupBy sqlA = new SQLAssembleQ_GroupBy(
				absConstant.getSQLDOC(),
				queryDiff,
				jsonObject.getJSONObject("condition"), 
				jsonObject.getJSONObject("operator"),
	...
	public String dfindArray() {
		...
		JSONObject jsonObject = JSONObject.fromObject(jdata);
//		SQLAssemble2 sqlA = new SQLAssemble2(jsonObject, "MigAuditcReportDetailResult");
		AbstractConstant absConstant = new ConstMigAuditcReportDetailResult();
...
//			JSONObject jsonObject = JSONObject.fromObject(jdata);
//			JdataChecker checker = new JdataChecker(jsonObject, "MigAuditcReportDetailResult");
