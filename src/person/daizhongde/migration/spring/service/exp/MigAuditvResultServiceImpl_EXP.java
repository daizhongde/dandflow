...
public class MigAuditvResultServiceImpl_EXP implements BaseExportService {

	private static final Log log = LogFactory.getLog(MigAuditvResultServiceImpl_EXP.class);
...
//			ExportSemaphore.level1.acquire();
	 ...
	 * @throws IOException
	 */
	private void generateXLS(String fAbsPath, 
...
//			ExportSemaphore.level1.acquire();
	...
	private void generateTXT(String fAbsPath, 
			AbstractConstant absConstant, 
			int total, 
			SQLAssembleE sqlA,
			String _, List customColumns, Map options ) throws IOException {
...
//					currentSheetRecordCount, bw, sheetNumber * pageSize );
	...
	public static BaseExportService getFromApplicationContext(
		...
		return (BaseExportService) ctx.getBean("migAuditvResultService_EXP");
	}
	@Override
...
//		String fileName = absConstant.getEXPfileName();
		String fileName = "KPI-Legality";
		
		AbstractConstant absConstant = new ConstMigAuditvResult();
		
		if( null == columnNames ){
			columnNames = absConstant
			.getJSDOC()
			.getExport()
			.get("export")
			.getDefaultColumns();
		...
		if( null == options ){
			options = EXPORT.xlsExportOptions;
		}else{
			HashMap copy = MapUtils.Map2HashMap(EXPORT.xlsExportOptions);
			copy.putAll(options);
			options = copy;
		}
...
//    	int level2MinCount = absConstant.getEXPlevel2MinCount()*INIT.IETxtMultiple;
//    	int level3MinCount = absConstant.getEXPlevel3MinCount()*INIT.IETxtMultiple;
		String queryDiff = ((SQLNode)absConstant.getSQLDOC().getExport()
				.get("export4KPIReport")
			).getSQL();
...
//    	HashMap<String,String> allColumnSQLMap = absConstant.getEXPAllColumnSQLMap();
		HashMap<String,String> allColumnSQLMap = JSQLParser.getSelectMap( queryDiff );
    	
		//customSelectSQL = customSelectCol+ from XX
		String customSelectSQL ="";
		String customSelectCol = "select ";
		/* because we are chinese, so column title need convert to chinese 
		 * customColumns [N_MID, C_MNAME,N_Mlevel,level...] */
		for(int i = 0, j = columnNames.size(); i<j; i++){
			if(i!=j-1){
				customSelectCol += allColumnSQLMap.get(columnNames.get(i))+",";
			}else{
				customSelectCol += allColumnSQLMap.get(columnNames.get(i))+" ";
			}
		}
		//像这样采用特殊的方式拼装的sql,下面这行就要改成这样
...
//		String tempFAbsName = absConstant.getEXPfileName() + String.valueOf(new java.util.Date().getTime()) + ".txt";
		SimpleDateFormat stm=new SimpleDateFormat("yyyyMMdd-HHmmss-S");
		String tempFAbsName = fileName +"_"+ stm.format(new Date()) + ".txt";
...
//      fileCount = (int) ( (total - 1) / fileRecordSize ) + 1;
		...
		HSSFWorkbookUtil4Export hwbUtil = new HSSFWorkbookUtil4Export(hwb, cs, sheetName,
				...
				absConstant.getEXPfixColumnCount(), true,
//				absConstant.isEXPsheetAutoSizeColumn(), 
				options );
...
//	    	int offset = (page - 1) * pageSize;// 第一条记录的索引,这个取自分页查询,因为这里sheetNumber是从零开始,所以不用减一;
			...
			hwbUtil.generateASheet(sheetNumber,
	    			list,
	    			currentSheetRecordCount );
			hwb.getSheetAt(sheetNumber).setColumnWidth(7, 60*256);;
