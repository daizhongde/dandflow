...
import person.daizhongde.migration.constant.ConstMigAuditcReportDetailResult;
...
public class MigAuditcReportDetailResultServiceImpl_EXP implements MigAuditcReportDetailResultService {

	private static final Log log = LogFactory.getLog(MigAuditcReportDetailResultServiceImpl_EXP.class);
	
	private MigAuditcReportDetailResultDAO dataDAO;
	
	public String exportXLS(String jdata, String _ ) throws Exception{
		log.debug("-->1");
		
		AbstractConstant absConstant = new ConstMigAuditcReportDetailResult();
...
//			ExportSemaphore.level1.acquire();
	...
	@Override
	public String exportXLS_Diff(String jdata, String _, List customColumns, Map options, String sql )
			throws Exception {
		log.debug("-->exportXLS_Diff");
    	
		JSONObject jsonObject = JSONObject.fromObject(jdata);
		
		AbstractConstant absConstant = new ConstMigAuditcReportDetailResult();
//		absConstant.setEXPfileName("一致性稽核差异报表");
//		absConstant.setEXPsheetName("C-Diff Report");
		String EXPsheetName = "C-Diff Report";
...
//		HashMap<String,String> allColumnSQLMap = absConstant.getEXPAllColumnSQLMap();
    	HashMap<String,String> allColumnSQLMap = JSQLParser.getSelectMap( sql ); 
//		Printer.printJSON(allColumnSQLMap);
...
//		customColumns [N_MID, C_MNAME,N_Mlevel,level...]
		...
		String tempFAbsName = "一致性稽核差异报表_" + stm.format(new Date()) + ".xls";
		...
		if( total > fileRecordSize ){
			...
			if(IESemaphore.level3.tryAcquire()){
				try{
					generateXLS( fAbsPath, absConstant, total, sqlA, _, customColumns, options, EXPsheetName );
				}catch(IOException e){
					throw e;
				}finally{
					IESemaphore.level3.release();
...
//			ExportSemaphore.level1.acquire();
	...
	@Override
	public String exportXLS_gbsubdomain(String jdata, String _, List customColumns, Map options, String sql )
			throws Exception {
		log.debug("-->exportXLS_gbsubdomain");
    	
		JSONObject jsonObject = JSONObject.fromObject(jdata);
		
		AbstractConstant absConstant = new ConstMigAuditcReportDetailResult();
//		absConstant.setEXPfileName("一致性稽核子域分组报表");
//		absConstant.setEXPsheetName("Group By Sub-domain");
		String EXPsheetName = "Group By Sub-domain";
...
//		customColumns [N_MID, C_MNAME,N_Mlevel,level...]
		...
		String tempFAbsName = "一致性稽核子域分组报表_" + stm.format(new Date()) + ".xls";
...
//			ExportSemaphore.level1.acquire();
	...
	public void generateXLS(String fAbsPath, 
		...
		generateXLS(fAbsPath, absConstant, total, sqlA, _, 
				customColumns, options, absConstant.getEXPsheetName() );
	}
	/**
	 * 
	 * @param fAbsPath
	...
	public void generateXLS(String fAbsPath, 
			...
			int total, 
			SQLAssembleE sqlA,
			String _, List customColumns, Map options, String EXPsheetName ) throws IOException {
    	
		log.debug("-->generateXLS");
				
    	String sheetName = EXPsheetName;//Excel中的表单名
//		String sheetName = "一致性稽核差异报表";
...
//					currentSheetRecordCount, bw, sheetNumber * pageSize );
	...
	public void setDataDAO(MigAuditcReportDetailResultDAO dataDAO) {
		this.dataDAO = dataDAO;
	}

	public static BaseExportService getFromApplicationContext(
			ApplicationContext ctx) {
		return (BaseExportService) ctx.getBean("migAuditcReportDetailResultService_EXP");
	}
	@Override
