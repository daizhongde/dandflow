...
/**
 * v2 
 * Only fix number task running as the same time, ignore dependences
 * @author daizd
 *
 */
public class MigJobProcessServiceImpl_v2 implements MigJobProcessService {
...
//	private Semaphore semp = new Semaphore(1);
	...
	private MigJobInsService migJobInsService;
...
//		migJobInsService.modifyJobStatus2init(jobInsId, "", null);
		...
		if(jobIns1.getStatus().equalsIgnoreCase( JobState.FINISH )
			...
			if( jobInsDAO.sqlQueryUpdateJobInsState_2Init(jobInsId) == 1 ){
				dataDAO.sqlQueryUpdateProcessStateByIns_2Init( jobInsId );
			}else{
				System.out.println("Instance is locked or running ");
				return;
			}
		}else if(jobIns1.getStatus().equalsIgnoreCase( JobState.INITIAL )){
			//不要重置，什么也不做
		}else{
			System.out.println("Only Finished or Paused instance can be initial! And Finished or Paused 、INITIAL's ins can be started by quartz schedule! ");
			return;//只有初始化、完成、暂停状态的示例才能进入下一步启动作业
		}
		
//		jobInsId="IS00000077";
		System.out.println("Schedule startJob jobInsId:"+jobInsId);
		MigJobIns jobIns = jobInsDAO.findById(jobInsId);
		String logname = jobIns.getAuthor();
...
//		log.debug("222");
	...
	private void checkJob(MigJobIns jobIns){
		...
		if( JobLockState.LOCK.equalsIgnoreCase( jobIns.getLockStatus() ) ){
			System.out.println("The job <"+jobInsId+"> is locked, can't start !");
		...
		if( JobState.EXECUTING.equalsIgnoreCase(status) 
			...
			System.out.println("The job <"+jobInsId+"> status is <"+status+">, can't start !");
		...
		if( JobState.ERROR.equalsIgnoreCase(status) 
			...
			if ( jobInsDAO.sqlQueryUpdateJobInsState( jobInsId,
				...
				System.out.println("Data synchronization, others are updating the data, update failed!");
...
//								JobSemaphore.maxThreadNum.release();//释放
							...
							JobSemaphore.maxThreadNum.acquire();//获得资源后才能更新状态
							int uRow = dataDAO.sqlQueryUpdateProcessState_init2Running( processId );
							if(uRow != 1){
								//叶子节点由初始状态置为运行态，如果更新不到，则返回
								log.debug("Thread " + this.i + ", proc_id<" + this.processId + ">,node<" + this.node + ">,nodelink<" + this.nodelink + "> Find the leaves task is executing, task deal done. " );
								JobSemaphore.maxThreadNum.release();//如果没有更新到记录就释放
								return;
							}
							/* 加到这里 解决下面的问题：
...
//		JobSemaphore.maxThreadNum.acquire();// 获取许可,在每个线程执行完的时候都一定要释放，且线程不能等待他的子线程
		if(JobSemaphore.maxThreadNum.availablePermits()>INIT.maxThreadNum){
			log.error("线程管理存在问题<信号量有被减成负数>，请统一把资源执行锁加在叶子任务前，并在叶子任务执行后解锁！");//当信号量被减成负数时
		}
//		log.debug("Thread "+ i + ": proc_id<" + procId
...
//	}
	...
	public void setMigJobInsService(MigJobInsService migJobInsService) {
		this.migJobInsService = migJobInsService;
	}
